...
def run(
    ...
    save_format=0,  # YOLO or Pascal-VOC for *.txt
    ...
    project=ROOT / "runs/detect",  # save results
    name="exp",  # save results
    exist_ok=False,  # existing project/name ok, do not increment
    line_thickness=3,  # bounding box thickness (pixels)
    hide_labels=False,  # hide labels
    hide_conf=False,  # hide confidences
    half=False,  # FP16 half-precision
    dnn=False,  # use OpenCV DNN for ONNX
    ...
    # JSON: структура, куда собираем результаты
    images_data = {}  # { "image_name.png": [ {...}, {...} ], "another.png": [...] }
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):
            ...
            # Словарь для группировки bbox по классам
            class_map = {}  # { class_id: { "obj_class_name":..., "obj_class_id":..., "coords": [...] } }
            ...
            if len(det):
                ...
                for *xyxy, conf, cls_ in reversed(det):
                    ...
                    # Сохраняем в class_map
                    if c not in class_map:
                        class_map[c] = {
                            "obj_class_name": label_name,
                            "obj_class_id": c,
                            "coords": []
                        }
                    class_map[c]["coords"].append({
                    ...
                    if save_txt:
                        ...
                        line = (cls_, *coords, conf) if save_conf else (cls_, *coords)
                        with open(f"{txt_path}.txt", "a") as f:
                            f.write(("%g " * len(line)).rstrip() % line + "\n")

                    if save_img or save_crop or view_img:
                        label = None if hide_labels else (str(c) if hide_conf else f"{c} {conf:.2f}")
                        annotator.box_label(xyxy, label, color=colors(c, True))
                    if save_crop:
                        save_one_box(xyxy, imc, file=save_dir / "crops" / label_name / f"{p.stem}.jpg", BGR=True)

            # Превращаем class_map -> список для JSON
            results_list = []
            for class_id, info in class_map.items():
                info["count"] = len(info["coords"])
                results_list.append(info)
            # Записываем
            images_data[p.name] = results_list
    ...
    # После обработки всех изображений -> сохраняем JSON
    final_list = [images_data]
    json_path = save_dir / "results.json"
    with open(json_path, "w", encoding="utf-8") as jf:
        json.dump(final_list, jf, indent=4)
...
def parse_opt():
    ...
    parser.add_argument("--save-format", type=int, default=0, help="0 for YOLO, 1 for Pascal-VOC when saving --save-txt")
